...
class ScaledYOLOV4:
    _defaults = {
        "triton_url" : "172.20.0.4:8001",
        "model_name" : "scaledyolov4",
        "model_version" : "1",
        "thresh": 0.4,
        "nms_thresh": 0.5,
        ...
        "half": False,
        "same_size": False,
        "classes_path": pkg_resources.resource_filename(
            "scaledyolov4", "data/coco.yaml"
        ),
    }
    ...
    def __init__(self, bgr=True, gpu_device=0, **kwargs):
        ...
        self.triton = tr.InferenceServerClient(self.triton_url)
        self.dtype = "FP32"
        # self.inputs = [
        #     tr.InferInput(
        #         "data",
        #         [1, 3, self.model_image_size, self.model_image_size],
        #         "FP32" if not self.half else "FP16",
        #     )
        # ]
        # self.outputs  = [tr.InferRequestedOutput("prob")]
    ...
    def _get_class(classes_path):
        ...
        if len(class_names) != num_classes:
            raise AssertionError(
                f"{len(class_names)} names found for nc={num_classes} dataset in {classes_path}"
            )
        return class_names
    ...
    def _detect(self, list_of_imgs):
        ...
        resized = [
            self._letterbox(img, new_shape=self.model_image_size, auto=self.same_size)[
                0
            ]
        ...
        for image in images:
                ...
                outputs = [tr.InferRequestedOutput("prob")]
                pred = self.triton.infer(
                    model_name=self.model_name,
                    model_version=self.model_version,
                    inputs=inputs,
                    outputs=outputs,
                ).as_numpy("prob")
                no_dets = int(pred[0, 0, 0, 0])
                bboxes = torch.from_numpy(pred[0, 1:(no_dets*6+1), 0, 0].reshape(1, -1, 6))
                # in yololayer.cu, they decided to already give us box_prob * class prob
                # (x, y, w, h, box conf * class conf, class id) 6 outputs for each det
                preds.append(bboxes)
        preds = torch.cat(preds, dim=0)
        return preds, input_shapes

    def detect_get_box_in(
        self, images, box_format="ltrb", classes=None, buffer_ratio=0.0, raw=False
    ):
        """
        Parameters
        ----------
        ...
        if raw:
            all_dets = self._postprocess_raw(
                res,
    ...
    def _postprocess(
        ...
        preds = non_max_suppression(
            boxes, self.thresh, self.nms_thresh, classes=class_idxs
        )
