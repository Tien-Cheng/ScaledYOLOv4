...
def test(data,
    ...
    path = data['test'] if opt.task == 'test' else data['val']  # path to val/test json
    ...
    for batch_i, (img, targets, paths, shapes, img_ids) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            if save_json:
                ...
                box = scale_coords(img[si].shape[1:], box, shapes[si][0], shapes[si][1])  # to original shape
    ...
    if save_json and len(jdict):
        f = str(save_dir / 'dets_%s_results.json') % \
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            cocoGt = COCO(path)  # initialize COCO ground truth api
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='inference', help='directory to save results')
    ...
    if opt.task in ['val', 'test']:  # run normally
        ...
        # for weights in ['']:
        for weights in ['runs/exp11_p5-til-backbone100/weights/last_p5-til-backbone100_strip.pt']:
            file = str(Path(opt.save_dir) / 'study_%s_%s.txt') % (Path(opt.data).stem, Path(weights).stem)  # filename to save to
            if Path(file).is_file():
                Path(file).unlink()
            # x = list(range(416, 897, 32))
